```admonish info
This page is partially adopted from [https://wiki.archlinux.org/title/Systemd-boot](https://wiki.archlinux.org/title/Systemd-boot)

We will be using `systemd-boot` as the boot loader.
See
[https://wiki.archlinux.org/title/Arch_boot_process#Boot_loader](https://wiki.archlinux.org/title/Arch_boot_process#Boot_loader)
for all options.
```
...
Assuming you have been following this guide, the ESP mount point should be at `/boot` right now (chrooted into the system i.e. `/mnt`)
...
```admonish warning
Note the `UUID=` part in the last line!!!
```
...
Then, reboot the machine, type `root` for the login, then type the root password you just set (not the `change` you set initially for the live environment).
...
You should see something like the below if you login successfully. `pistonite` would be your hostname.
```
[root@pistonite ~]#
```
